...
Passive (as well as active) transports are only possible between containers that are connected via a neighborhood relation in the spatial structure of the model. The rules for **target** and **source** containers of a transport are defined by the _container criteria_ as a mandatory condition.
