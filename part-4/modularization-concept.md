...
{% hint style="warning" %}
If module `B` defines the neighborhood `N1` with a neighbor that cannot be found in the final model structure, the simulation creation fails with an error. To **remove** a neighborhood defined in a previous module, redefine it **without neighbors** - with *both* neighbors empty, see [Removing a neighborhood defined in another module](spatial-structures-bb.md#removing-a-neighborhood-defined-in-another-module). The neighborhood is then removed from the simulation and listed in a dialog after the simulation has been created, updated or configured. The removal works in both merge behaviors.
{% endhint %}
