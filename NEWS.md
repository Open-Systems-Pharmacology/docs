...
- **`MoleculeProperties` of the spatial structure now follow the module's merge behavior.** In v12 they were always merged, whatever the merge mode was set to. In v13, "Extend" merges them - a property present in both modules takes the later module's value or formula - while "Overwrite" **replaces** the accumulated container, so only the properties defined in the overwriting module survive. An overwriting module that contributes an *empty* `MoleculeProperties` container therefore clears the accumulated properties. Watch the **top-level** container in particular: every spatial structure has one, it is invisible while empty and cannot be deleted, so any module carrying a Spatial Structure building block wipes the earlier modules' molecule properties as soon as it is set to "Overwrite". Inside containers the trap is gone, because MoBi® no longer adds `MoleculeProperties` automatically ([MoBi #2498](https://github.com/Open-Systems-Pharmacology/MoBi/issues/2498)). See [Spatial structure](part-4/converting-v12-projects-to-v13.md#spatial-structure). ([Core #2944](https://github.com/Open-Systems-Pharmacology/OSPSuite.Core/issues/2944), [MoBi #2472](https://github.com/Open-Systems-Pharmacology/MoBi/issues/2472))
...
- **Under "Extend", a redefinition by the extending module now takes effect everywhere.** "Extend" merged child entities (parameters, molecule lists) correctly, but several *properties* of a redefined entity were silently kept from the earlier module, so half of a redefinition was dropped with nothing in the interface indicating it. All of the following now follow the extending module, with container criteria extended (conditions combined, operator overwritten) exactly as for passive transports:
  ...
  - **Observers** - the container criteria of "In container with", plus the dimension, description and icon. Previously the extending module's equation was evaluated in the earlier module's set of containers, and a changed dimension was ignored, so the observer's equation and its dimension no longer matched. ([Core #2950](https://github.com/Open-Systems-Pharmacology/OSPSuite.Core/issues/2950))
  - **Reactions** - the container criteria are now *extended* rather than replaced. This one was actively dangerous: an empty criteria means "no restriction" for a reaction, so an extending module that redefined a reaction without touching its criteria - the default in MoBi® - silently dropped the earlier module's restriction and created the reaction in every container where its educts, products and modifiers were present. ([Core #2949](https://github.com/Open-Systems-Pharmacology/OSPSuite.Core/issues/2949))
...
- **`MoleculeProperties` are added deliberately** - a `MoleculeProperties` container is no longer created automatically when a container is added to a spatial structure or switched to physical. Add one with the new **Add MoleculeProperties** entry in the container's context menu; switching the container to logical removes it again. This also removes a merge hazard: a stray empty `MoleculeProperties` container in a module set to "Overwrite" clears the molecule properties of the earlier modules. ([MoBi #2498](https://github.com/Open-Systems-Pharmacology/MoBi/issues/2498))
