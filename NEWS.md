...
- **`MoleculeProperties` of the spatial structure now follow the module's merge behavior.** In v12 they were always merged, whatever the merge mode was set to. In v13, "Extend" merges them - a property present in both modules takes the later module's value or formula - while "Overwrite" **replaces** the accumulated container, so only the properties defined in the overwriting module survive. An overwriting module that contributes an *empty* `MoleculeProperties` container therefore clears the accumulated properties — delete such a container from the module, or set the module to "Extend". MoBi® no longer creates `MoleculeProperties` containers by itself, so one is only present where it was added deliberately; a module carried over from v12 may still contain one that the v12 user interface created automatically and that was never filled. See [Spatial structure](part-4/converting-v12-projects-to-v13.md#spatial-structure). ([Core #2944](https://github.com/Open-Systems-Pharmacology/OSPSuite.Core/issues/2944), [MoBi #2472](https://github.com/Open-Systems-Pharmacology/MoBi/issues/2472))
- **Neighborhoods can be removed, and unresolvable neighbors now fail the build.** A later module that redefines a neighborhood **without neighbors** removes it from the simulation - in v12 a neighborhood could not be removed at all. MoBi® accepts a neighborhood with both neighbors left empty, and lists every removed neighborhood in a dialog after a simulation has been created, updated or configured. A neighborhood whose neighbors are not present in the final model structure now **fails simulation creation** instead of being silently skipped. See [Removing a neighborhood defined in another module](part-4/spatial-structures-bb.md#removing-a-neighborhood-defined-in-another-module) and [Spatial structure](part-4/converting-v12-projects-to-v13.md#spatial-structure). ([Core #2918](https://github.com/Open-Systems-Pharmacology/OSPSuite.Core/issues/2918), [MoBi #2479](https://github.com/Open-Systems-Pharmacology/MoBi/issues/2479))
...
- **Changed event combination under "Extend".** When two modules define an equally-named event or application administering different molecules, v12 "Extend" produced a malformed event administering both; in v13 the administered molecule is taken from the later module, consistent with the precedence of other overwritten properties. See [Events](part-4/converting-v12-projects-to-v13.md#events).
- **Under "Extend", the extending module also wins for the properties of the redefined entity.** Child entities such as parameters and molecule lists are merged. In addition, the following properties are taken from the extending module, with container criteria extended (conditions combined, operator overwritten) exactly as for passive transports:
  - **Events** - the start condition equation and the "One Time" flag; for a transport inside an event or application, the kinetic formula, the source/target criteria and the "Create/Plot process rate parameter" flags. ([Core #2943](https://github.com/Open-Systems-Pharmacology/OSPSuite.Core/issues/2943))
  - **Molecules** - for an active transport process under a transporter molecule, the kinetic formula, the source/target criteria, the "Create/Plot process rate parameter" flags and the transport name. ([Core #2951](https://github.com/Open-Systems-Pharmacology/OSPSuite.Core/issues/2951))
  - **Observers** - the container criteria of "In container with", plus the dimension. ([Core #2950](https://github.com/Open-Systems-Pharmacology/OSPSuite.Core/issues/2950))
  - **Reactions** - the container criteria. Note that an empty container criteria means "no restriction" for a reaction, so a reaction redefined in an extending module without container criteria of its own keeps the restriction of the earlier module instead of being created in every container where its educts, products and modifiers are present. ([Core #2949](https://github.com/Open-Systems-Pharmacology/OSPSuite.Core/issues/2949))
...
![The Commit simulation parameters to compound dialog.](https://raw.githubusercontent.com/Open-Systems-Pharmacology/docs/v13/assets/images/part-3/overwrite-parameter-sets-commit-dialog.png)
...
![The Simulation Explorer showing the orange uncommitted-changes overlay on the simulation, on a compound that is otherwise in sync, and on a compound that is also out of sync with its building block.](https://raw.githubusercontent.com/Open-Systems-Pharmacology/docs/v13/assets/images/part-3/overwrite-parameter-sets-uncommitted-indicator.png)
...
![The Overwrite Parameter Sets tab of a compound, with the list of sets on the left and the metadata and parameter values of the selected set on the right.](https://raw.githubusercontent.com/Open-Systems-Pharmacology/docs/v13/assets/images/part-3/overwrite-parameter-sets-compound-tab.png)
...
![The Compounds tab of the Configure Simulation window, with the Overwrite Parameter Set selection for one compound.](https://raw.githubusercontent.com/Open-Systems-Pharmacology/docs/v13/assets/images/part-3/overwrite-parameter-sets-selection.png)
...
The feature is described in detail in [Overwrite Parameter Sets](https://docs.open-systems-pharmacology.org/v13/working-with-pk-sim/pk-sim-documentation/pk-sim-overwrite-parameter-sets).
...
- **Snapshots of single building blocks** - every building block can now be exported to its own snapshot file with **Save Snapshot...**, and building blocks can be loaded from such a file with **Load from Snapshot...** on the corresponding building block folder. This works for every building block type - Compounds, Individuals, Populations, Expression Profiles, Administration Protocols, Formulations, Events and Observer Sets - as well as for observed data. See [Snapshots](https://docs.open-systems-pharmacology.org/v13/working-with-pk-sim/pk-sim-documentation/pk-sim-snapshots#snapshots-of-single-building-blocks). ([PK-Sim #1312](https://github.com/Open-Systems-Pharmacology/PK-Sim/issues/1312))
...
### MoBi®: Project snapshots

MoBi® can now export a project to a JSON **project snapshot** and re-create a project from one, as PK-Sim® does. The snapshot stores the inputs rather than the built models: PK-Sim® modules are stored as their PK-Sim® snapshot and re-created by a local PK-Sim® installation on load, extension modules are stored as PKML, and each simulation is stored as its configuration together with the values changed by the user. Individuals and Expression Profiles coming from PK-Sim® are stored as their PK-Sim® snapshot plus the changes made in MoBi® afterwards. Modules, Individuals and Expression Profiles that carry a PK-Sim® snapshot can also be exported and rebuilt individually from their context menu. See [Snapshots](https://docs.open-systems-pharmacology.org/v13/working-with-mobi/mobi-documentation/mobi-snapshots). ([MoBi #1917](https://github.com/Open-Systems-Pharmacology/MoBi/issues/1917), [MoBi #1919](https://github.com/Open-Systems-Pharmacology/MoBi/issues/1919), [MoBi #1921](https://github.com/Open-Systems-Pharmacology/MoBi/issues/1921), [MoBi #2110](https://github.com/Open-Systems-Pharmacology/MoBi/issues/2110), [MoBi #646](https://github.com/Open-Systems-Pharmacology/MoBi/issues/646), [MoBi #2101](https://github.com/Open-Systems-Pharmacology/MoBi/issues/2101))
...
### MoBi®: Selecting where simulation changes are committed

Changes to parameter values and initial conditions were previously always committed to the building blocks of the **last** module in the simulation configuration. MoBi® now asks for the commit target: the **module** that receives the changes and, within it, the **Parameter Values** and **Initial Conditions** building block to write them to - or a newly created one. The dialog offers the last module and the building blocks the simulation currently uses by default, so confirming it commits to the same target as before.

Committing to a building block that the simulation does not use writes the values to that building block alone; the simulation is not updated and remains marked as out of sync. MoBi® warns about this, and also when the committed values would be shadowed by a building block of a later module in the configuration. See [Committing changes from a simulation to a building block](https://docs.open-systems-pharmacology.org/v13/working-with-mobi/mobi-documentation/modularization-concept#committing-changes-from-a-simulation-to-a-building-block). ([MoBi #1614](https://github.com/Open-Systems-Pharmacology/MoBi/issues/1614))
...
- **`MoleculeProperties` containers are added and removed explicitly.** They are no longer created automatically - neither at the top level of a new Spatial Structure building block nor when a container is created or switched to physical. A new **Add MoleculeProperties** context-menu entry on the spatial structure node and on physical containers creates one where it is missing, and a `MoleculeProperties` container can now be **deleted** from its own context menu. Both actions can be undone. Together with the new merge behavior this keeps an empty container from silently clearing the molecule properties of earlier modules. ([MoBi #2512](https://github.com/Open-Systems-Pharmacology/MoBi/issues/2512), [MoBi #2498](https://github.com/Open-Systems-Pharmacology/MoBi/issues/2498), [MoBi #2514](https://github.com/Open-Systems-Pharmacology/MoBi/issues/2514))
- **New option "Warn for non-finite quantities"** - if enabled, a warning is shown in the **Notifications** view for each quantity whose value is `NaN` or infinite at time `t=0`. The check is performed when a simulation is created and each time it is run. See [Options](https://docs.open-systems-pharmacology.org/v13/working-with-mobi/mobi-documentation/mobi-options). ([MoBi #2511](https://github.com/Open-Systems-Pharmacology/MoBi/issues/2511))
...
- **Snapshot and project loading** - simulations can be loaded from a MoBi® [snapshot](https://docs.open-systems-pharmacology.org/v13/working-with-mobi/mobi-documentation/mobi-snapshots), and projects can be opened without loading their simulations. ([MoBi #2402](https://github.com/Open-Systems-Pharmacology/MoBi/issues/2402), [MoBi #1908](https://github.com/Open-Systems-Pharmacology/MoBi/issues/1908))
