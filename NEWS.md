...
- **Under "Extend", a redefinition by the extending module now takes effect everywhere.** "Extend" merged child entities (parameters, molecule lists) correctly, but several *properties* of a redefined entity were silently kept from the earlier module, so half of a redefinition was dropped with nothing in the interface indicating it. All of the following now follow the extending module, with container criteria extended (conditions combined, operator overwritten) exactly as for passive transports:
  ...
  - **Observers** - the container criteria of "In container with", plus the dimension, description and icon. Previously the extending module's equation was evaluated in the earlier module's set of containers, and a changed dimension was ignored, giving a dimensionally inconsistent observer. ([Core #2950](https://github.com/Open-Systems-Pharmacology/OSPSuite.Core/issues/2950))
